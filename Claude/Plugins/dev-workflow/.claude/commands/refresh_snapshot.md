...
If no state file is found, stop and tell the user: "No active workflow found. Nothing to refresh — run `/start-ticket-analysis` to begin."
...
2. Re-read the files listed in the existing `codebase_context.md` relevant file map to check for changes — use Glob and Grep to find any new files related to the active ticket scope.
3. Note any files that have been added, removed, or significantly changed since the previous snapshot (you can compare against what the old snapshot listed).
...
Then say: "Snapshot is up to date. Continue with `/approve-step` to implement the next step, or run `/submit-review-feedback` if the pulled changes affect your implementation plan."
